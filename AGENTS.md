...
`isUnilateral` is a set *shape*, not a display toggle: flipping it silently doubles or halves every historical volume number for the exercise, and once sets carry per-side columns (`repsLeft`/`weightRight`/…) it also strands side data on a now-bilateral exercise, or leaves a now-unilateral exercise whose past sets have no sides. An exercise that changed shape is a *different* exercise — make a new one.
