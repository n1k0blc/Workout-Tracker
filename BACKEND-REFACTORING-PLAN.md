...
### PR 3 — Analytics + dashboard consolidation — 🚧 in progress, backend + frontend implementation done, pending manual verification (2026-07-05)

**Backend**: rewrote `analytics.service.ts`/`analytics.controller.ts`/`analytics/dto/*` from scratch around a shared `AnalyticsFilterDto` (`common/dto/analytics-filter.dto.ts`) and a new `loadWorkoutsForAnalytics` loader.

- **One endpoint per metric** (10 total: `volume`, `prs`, `muscle-distribution`, `cycles`, `rir`, `duration`, `rest-time`, `reps`, `sets`, `intensity`), replacing the old 15 (9 metrics × base+`-by-cycle` twin, minus the 3 already-dropped ORM routes). `filter.cycleId` presence alone switches a metric into cycle-anchored mode (cycle-relative week bucketing + `trainingDay` counter, ownership-verified via `workoutCycle.findFirst({id,userId})` → `NotFoundException`); its absence uses `period`/`startDate`/`endDate`, optionally narrowed via `scope: 'all'|'non-cycle'`. `getPersonalRecords` and `getCycles` were left outside the loader by design — PRs has no period/cycle concept (all-time scan, home-gym-only default) and cycles is a pure list.
- **Intensity** (`getIntensityAnalytics`, new): `3000 / (30 + reps + rir)` per working set, averaged per workout then overall — weight-independent by design, so unlike PRs it applies to every workout and every gym. `rir` null defaults to 0.
- **Duration** absorbed the old separate `time-tracking` endpoint (both were workout-level-duration views of the same data).
- **RIR** kept its bespoke inline 3-counter (rir0/1/2) week-aggregation loop rather than forcing it through the generic single-metric `aggregateByWeek<T>` helper, since that helper only sums one field.
- **Timezone fix** (§2.11): `getWeekBounds`/`getCalendarWeek` converted from local-time Date methods (`getDay`/`setDate`/`setMonth`) to their UTC equivalents, matching the UTC basis every data-point label already uses (`toISOString().split('T')[0]`) — previously a workout near local midnight could bucket into a different week than its displayed date implied. `getCycleWeekNumber` needed no change (already a pure epoch-ms diff). The two bare `new Date()` calls (`getDateFilter`, `getPersonalRecords`'s 30-day cutoff) now go through the MOCK_DATE-aware `getCurrentDate()`.
- **Tests**: new `analytics.service.spec.ts` (16 tests) covering the BOLA cycle-ownership guard, gym-filter 3-way branching, scope=non-cycle, volume's warmup-exclusion/unilateral/double-weight multipliers and muscle distribution, the Intensity formula (including its weight-independence), RIR bucketing, PR max-weight tracking + home-gym default, and the UTC week-bucketing fix. Full backend suite: 49/49 passing, `tsc --noEmit` and `nest build` both clean.

**Frontend**: `types/index.ts` and `lib/api/client.ts` collapsed every `*ByCycleAnalytics` type/method into its base counterpart (added `cycleId?`/`cycleName?`/week fields), added `IntensityAnalytics`/`getIntensityAnalytics`. `app/analytics/page.tsx` and `app/cycles/[id]/page.tsx` (which independently duplicate the same filter/chart logic, as before) had every `getXByCycle(...)` call site replaced with the base method plus `cycleId`, and gained a new "Intensität" view button + chart (mirroring the Reps/Sets single-line chart pattern, unit `%`), re-adding the metric removed from the UI in 2d.9. `tsc --noEmit` and `next build` both clean across the whole frontend.

**Not yet done**: manual browser verification of the rewritten analytics/cycle-detail pages (both time-mode and cycle-mode, all metrics including the new Intensity chart) — pending user testing, following the same pattern as PR 2.
